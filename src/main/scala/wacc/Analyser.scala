...
object Analyser {
    ...
    private def getUnOpType(op: UnaryOp, expression: Expression)(implicit errorList: List[Error]): Either[SAType, List[Error]] = {
        op match {
    ...
    private def collectErrors[A](a: List[A], collector: A => List[Error])(implicit errorList: List[Error]): List[Error] = {
      a match {
