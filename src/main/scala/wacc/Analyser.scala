...
object Analyser {
    ...
    private def getTypeIfExpressionsType(expressions: List[Expression], t: SAType, o: SAType)(implicit errorList: List[Error]): Either[SAType, List[Error]] = {
      expressions match {
    ...
    private def equalsTypeNoError(pos: Position, firstType: SAType, secondType: SAType)(implicit errorList: List[Error]): List[Error] = equalsType(firstType, secondType) match {
        case true => { 
          errorList
        }
        case default => {
          errorList :+ TypeError(pos, firstType.toString, List(secondType.toString))
        }
      } 

    // Position helpers 

    private def getExpressionPos(expression: Expression): Position = expression match {
    ...
    private def collectErrors[A](a: List[A], collector: A => List[Error])(implicit errorList: List[Error]): List[Error] = a match {
        case head :: b => collectErrors(b, collector)(errorList :++ collector(head))
        case Nil => errorList
    }
    

    private def collectErrors2[A, B](a: List[(A, B)], collector: (A, B) => List[Error])(implicit errorList: List[Error]): List[Error] = {
      a match {
        case (a1, b1) :: c => collectErrors2(c, collector)(errorList :++ collector(a1, b1))
        case Nil => errorList
      }
    }
    ...
    private def checkArrayConstraints(rvalue: RValue, list: List[Expression], expectedType: SAType, expectedArity: Int)(implicit errorList: List[Error]): List[Error] = {
        list match {
            case (id @ Identifier(_)) :: next => symbolTable.lookupVar(id) match {
                case Left(SAArrayType(actualType, innerArity)) => {
                    val equalsTypeErrors = equalsTypeNoError(getRValuePos(rvalue), actualType, expectedType)
                    if (!(equalsTypeErrors eq errorList)) {
                      return equalsTypeErrors
                    } else if ((innerArity + 1) != expectedArity) {
                      return (errorList :+ TypeError(getRValuePos(rvalue), SAArrayType(actualType, innerArity + 1).toString, List(SAArrayType(expectedType, expectedArity).toString)))
                    } 
                    return checkArrayConstraints(rvalue, next, expectedType, expectedArity)
                }
                case Left(t) => errorList :+ TypeError(getRValuePos(rvalue), t.toString, List(SAArrayType(expectedType, expectedArity).toString))
                case Right(el) => el
            }
            case (e:Expression) :: next => getExpressionType(e) match {
                case Left(expType) => errorList :+ TypeError(getRValuePos(rvalue), expType.toString, List(SAArrayType(expectedType, expectedArity).toString))
    ...
    private def checkRValue(rvalue: RValue, typeName: SAType)(implicit errorList: List[Error]): List[Error] = {
        rvalue match {
            case NewPair(fst, snd) => typeName match {
                case SAPairType(fstType, sndType) => checkExpression(snd, sndType)(checkExpression(fst, fstType))
                case default => errorList :+ NewPairError(getRValuePos(rvalue), "cannot create new pair of type" + typeName)
            }
            case al @ ArrayLiteral(list) => typeName match {
                case SAArrayType(arrayType: SAType, 1) => collectErrors(list, (x: Expression) => (checkExpression(x, arrayType)))
                case SAArrayType(arrayType: SAType, x) => checkArrayConstraints(rvalue, list, arrayType, x)
                case default => errorList :+ ArrayLiteralError(al.pos, "invalid array literal of type" + typeName)
            }
    ...
    private def checkDeclarationStatement(typeName: SAType, identifier: Identifier, rvalue: RValue)(implicit errorList: List[Error]): List[Error] = {
        return checkRValue(rvalue, typeName)(symbolTable.insertVar(identifier, typeName))
    }
    ...
    private def getPairElemType(index: PairIndex, pair: LValue)(implicit errorList: List[Error]): Either[SAType, List[Error]] = {
        ...
        typeName match {
            case Left(SAPairType(fstType, sndType)) => index match {
                case Fst => Left(fstType)
                case Snd => Left(sndType)
            }
            case Left(SAPairRefType) => Left(SAUnknownType)
            case Left(t) => Right(errorList :+ TypeError(getLValuePos(pair), t.toString, List(SAPairType.toString, SAPairRefType.toString)))
            case rel @ Right(_) => rel
        }
    }
    
    
    private def checkPairElem(index: PairIndex, pair: LValue, typeName: SAType)(implicit errorList: List[Error]): List[Error] = getPairElemType(index, pair) match {
            case Left(pairElemType) => equalsTypeNoError(getLValuePos(pair), pairElemType, typeName)
    ...
    private def checkIfStatement(condition: Expression, thenStatements: List[Statement], elseStatements: List[Statement])(implicit returnVal: SAType, errorList: List[Error]): List[Error] = {
      ...
      scoper.enterScope()
      val thenErrors = collectErrors(thenStatements, checkStatement)(conditionError)
      scoper.exitScope()
      scoper.enterScope()
    ...
    private def isValidExpression(expression: Expression)(implicit errorList: List[Error]): List[Error] = checkExpression(expression, SAAnyType)

    private def checkLValue(lvalue: LValue, typeName: SAType)(implicit errorList: List[Error]): List[Error] = {
        lvalue match {
            case id @ Identifier(_) => symbolTable.lookupVar(id) match {
    ...
    private def checkWhileStatement(condition: Expression, doStatements: List[Statement])(implicit returnVal: SAType, errorList: List[Error]): List[Error] = {
      ...
      if (!(conditionError eq errorList)) return conditionError
      scoper.enterScope()
      val statementErrors = collectErrors(doStatements, checkStatement)(conditionError)
      scoper.exitScope()
      statementErrors
    }
    ...
    private def checkStatement(statement: Statement)(implicit returnVal: SAType, errorList: List[Error]): List[Error] = statement match {
        ...
        case ReturnStatement(expression) => returnVal match {
            case SAAnyType => errorList :+ ReturnFromMainError(getExpressionPos(expression))
    ...
    private def bothTypesMatch(lhs: Expression, rhs: Expression, validTypes: List[SAType])(implicit errorList: List[Error]): Either[SAType, List[Error]] = validTypes match {
        case t :: tail => {
            getTypeIfExpressionsType(List(lhs, rhs), t, t) match {
                case l @ Left(_) => getExpressionType(lhs) match {
                    case Left(ll) => getExpressionType(rhs) match {
                        case Left(lr) if equalsType(ll, lr) => l
                        case Left(lr) => Right(errorList :+ TypeError(getExpressionPos(lhs), ll.toString, List(lr.toString)))
                        case rel @ Right(_) => rel
                    }
                    case rel @ Right(_) => rel
                }
    ...
    private def getArrayElemType(id: Identifier, indices: List[Expression])(implicit errorList: List[Error]): Either[SAType, List[Error]] =
        symbolTable.lookupVar(id) match {
            case Left(SAArrayType(arrayType, arity)) => {
                val indicesError = collectErrors(indices, (x: Expression) => checkExpression(x, SAIntType))
    ...
    private def getUnOpType(op: UnaryOp, expression: Expression)(implicit errorList: List[Error]): Either[SAType, List[Error]] = op match {
        case Negation => getTypeIfExpressionsType(List(expression), SAIntType, SAIntType)
        case Chr => getTypeIfExpressionsType(List(expression), SAIntType, SACharType)
        case Len => expression match {
            case id @ Identifier(_) => symbolTable.lookupVar(id) match {
                case Left(SAArrayType(_, _)) => Left(SAIntType) 
                case rel @ Right(_) => rel
            }
            case ArrayElem(id, indices) => symbolTable.lookupVar(id) match {
                case Left(SAArrayType(_, arity)) => if (indices.length < arity) {
                    Left(SAIntType) 
                } else Right(errorList :+ ArrayArityError(getExpressionPos(expression), indices.length, arity))
                case rel @ Right(_) => rel
            }
            case default => Right(errorList :+ TypeError(getExpressionPos(expression), expression.toString, List(SAArrayType.toString)))
        }
        case Not => getTypeIfExpressionsType(List(expression), SABoolType, SABoolType)
        case Ord => getTypeIfExpressionsType(List(expression), SACharType, SAIntType)
    }

    private def getBinOpType(binaryOp: BinaryOpApp)(implicit errorList: List[Error]): Either[SAType, List[Error]] = binaryOp.op match {
        case Mul | Div | Mod | Plus | Minus => getTypeIfExpressionsType(List(binaryOp.lhs, binaryOp.rhs), SAIntType, SAIntType)
        case And | Or => getTypeIfExpressionsType(List(binaryOp.lhs, binaryOp.rhs), SABoolType, SABoolType)
        case Gt | Ge | Lt | Le => {
          bothTypesMatch(binaryOp.lhs, binaryOp.rhs, List(SAIntType, SACharType)) match {
            case Left(_) => Left(SABoolType)
            case default => Right(errorList :+ BinaryOpAppTypeError(binaryOp.pos, List(SAIntType.toString, SACharType.toString)))
          }
        }
        case Eq | Neq => {
    ...
    private def getExpressionType(expression: Expression)(implicit errorList: List[Error]): Either[SAType, List[Error]] = expression match {
    ...
    private def checkExpression(expression: Expression, t: SAType)(implicit errorList: List[Error]): List[Error] = getExpressionType(expression) match {
        case Left(otherType) => equalsTypeNoError(getExpressionPos(expression), t, otherType)
        case Right(el) => el
    }
    ...
    // FUNCTION RELATED 
    private def mapDefs(function: Func)(implicit errorList: List[Error]): List[Error] = {
    ...
    private def checkFunction(func: Func)(implicit errorList: List[Error]): List[Error] = {
        def collectErrorsFunctionStatements(a: List[Statement], returnVal: SAType)(implicit errorList: List[Error]): List[Error] = {
          a match {
            case a :: b => collectErrorsFunctionStatements(b, returnVal)(checkStatement(a)(returnVal, errorList))
            case Nil => errorList
          }
        }
        scoper.enterScope()
        ...
        val paramErrors = collectErrors(params, (p: Parameter) => symbolTable.insertVar(p.identifier, convertSyntaxToTypeSys(p.typeName)))
        ...
        val funcErrors = collectErrorsFunctionStatements(func.body, functionReturnType)
        scoper.exitScope()
        scoper.exitScope()
        funcErrors
    }

    private def checkFunctions(program: Program)(implicit errorList: List[Error]): List[Error] = { 
      collectErrors(program.functions, checkFunction)(collectErrors(program.functions, mapDefs))
    }
